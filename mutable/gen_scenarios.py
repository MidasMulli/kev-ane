...
f=open(OUT,"a"); done=0; t0=time.time(); rng=random.Random(11)
while done<N:
    ...
    try:
        ...
        f.write(json.dumps({"family":f"fam{done+SKIP:04d}","seed":seed,"root":root,"files":files,"subdirs":subdirs,"ops":ops})+"\n"); f.flush(); done+=1
