"""I2 train (Astra-signed): 160 TRAIN genuine ISDA docs, labels = ANCHORED heading paragraphs (i2_common.evidence), 12-output head.
...
sys.path.insert(0, ROOT + "/isda2"); from i2_common import NAMES as FIELDS, evidence as i2_evidence
STEPS, B = 3000, 16
log = open("i2_train_raw.jsonl", "w")
...
assert not sh("pgrep -f 'i2_test.py|vf_train.py|t1_train.py|i1_train.py'"), "another GPU job is running"
...
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("i2_red_sentinel.log")])
time.sleep(3); assert sent.poll() is None
U = json.load(open("isda2_corpus241.json")); SPL = json.load(open("i2_split.json"))
...
for step in range(1, STEPS + 1):
    ...
    if step % 500 == 0 or step == STEPS or due_kill or el >= 5400:
        ...
        if h > best[0]:
            best = (h, step); mx.savez("i2_best.npz", **dict(tree_flatten(net.trainable_parameters())))
            pass
        if due_kill or el >= 5400 or step == STEPS:
            if not kill_checked:
                kill_checked = True
                if ci[1] < 0.70: killed = True; emit(event="I2_KILL", step=step, hit2=h, ci=ci, note="diagnostic, not a capability verdict"); break
        if el >= 5400: emit(event="hard_stop_90min", step=step); break

emit(event="I2_TRAIN_DONE", best_step=best[1], dev_hit3=best[0], elapsed_s=time.time() - t0)
sent.terminate()
