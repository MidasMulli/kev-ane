"""A5 scoring (usage: a5_score.py DIR MAX_N [--need-a3] [--interim]). --interim = futility look on papers n < MAX_N: stop iff upper < -0.05. Derived from: A'' scoring (Astra-signed): run ONLY after every a1/a2 (and needed a3) verdict is filed. Majority of three where 1 and 2 differ;
...
D = sys.argv[1]; MAXN = int(sys.argv[2]); items = sorted(os.path.basename(p)[:-5] for p in glob.glob(f"{D}/packets/*.json"))
...
R = [json.loads(l) for l in open("a5_test_raw.jsonl")]; cmp_ = [r for r in R if r["event"] == "compare" and r["n"] < MAXN]
...
FUT = ci[1] < -0.05
print(json.dumps(dict(event="A5_INTERIM" if "--interim" in sys.argv else "A5_RESULT", papers=len(cmp_), FUTILITY_STOP=FUT if "--interim" in sys.argv else None, n_questions=N, agreement=float(np.mean([x for c in cmp_ for x in c["agree"]])), disagreements=len(items),
