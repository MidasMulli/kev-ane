"""A5 train (Astra-signed): AX2 = CUA architecture, 12-output head, labels = window overlaps the FROZEN anchored evidence span
(a5_common.evidence). TRAIN 160 / DEV 25. Adapted from i2_train.py. DEV every 500 steps: hit@3 over DEV fields with an anchor,
paper-clustered bootstrap. KILL (diagnostic) at the first DEV eval >= 60 min if the UPPER 95% < 0.70. Hard stop 3,000 steps / 90 min."""
...
sys.path.insert(0, ROOT + "/a5"); from a5_common import NAMES as FIELDS, evidence as i2_evidence
STEPS, B = 3000, 16
log = open("a5_train_raw.jsonl", "w")
...
assert not sh("pgrep -f 'a5_test.py|a4b_test.py|i3_test.py'"), "another GPU job is running"
...
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("a5_red_sentinel.log")])
time.sleep(3); assert sent.poll() is None
U = json.load(open("a5_corpus.json")); SPL = json.load(open("a5_split.json"))
...
for step in range(1, STEPS + 1):
    ...
    if step % 500 == 0 or step == STEPS or due_kill or el >= 5400:
        h, ci, _ = dev_eval(); emit(event="dev", step=step, elapsed_s=time.time() - t0, hit2=h, ci=ci)
        if h > best[0]:
            best = (h, step); mx.savez("a5_best.npz", **dict(tree_flatten(net.trainable_parameters())))
            pass
        if due_kill or el >= 5400 or step == STEPS:
            if not kill_checked:
                kill_checked = True
                if ci[1] < 0.70: killed = True; emit(event="A5_KILL", step=step, hit2=h, ci=ci, note="diagnostic, not a capability verdict"); break
        if el >= 5400: emit(event="hard_stop_90min", step=step); break

emit(event="A5_TRAIN_DONE", best_step=best[1], dev_hit3=best[0], elapsed_s=time.time() - t0)
sent.terminate()
