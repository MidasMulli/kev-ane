"""I3 TEST (Astra-signed): 56 FRESH EDGAR ISDA docs (seed 20260933 from edgar_pool.json), adapter IS3, corrected anchors, keyword sidecar.
...
sys.path.insert(0, ROOT + "/cuad"); os.chdir(ROOT + "/isda3"); sys.path.insert(0, ROOT + "/isda3"); sys.path.insert(0, ROOT + "/isda2")
from ane_run import Ane, export_and_pack, sh, MD, TRUST
from i3_common import NAMES, prompt, parse, norm as i2norm, sidecar
P = json.load(open("edgar_pool.json")); SPLIT = dict(test=sorted(random.Random(20260933).sample(range(len(P)), 56)))
json.dump(SPLIT, open("i3_test_split.json", "w"))
from transformers import AutoTokenizer
log = open("i3_test_raw.jsonl", "w")
...
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("i3_test_red_sentinel.log")]); time.sleep(3); assert sent.poll() is None
...
def windows(text):
    ...
    return out
head, amd5 = export_and_pack("i3_best.npz", "IS3"); md5_0 = sh(f"sudo -n md5 -q {MD}/weights/weight.bin")
ane = Ane("IS3", head)
# ---- G-SWAP ----
...
s1 = hs_sha("CUA"); s2 = hs_sha("IS3"); s3 = hs_sha("CUA"); s4 = hs_sha("IS3")
...
ane.close()
emit(event="I3_RUN_DONE", G_SWAP=g_swap, base_md5_unchanged=sh(f"sudo -n md5 -q {MD}/weights/weight.bin") == md5_0)
sent.terminate()
