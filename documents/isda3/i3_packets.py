...
D = "adjudication_i3"; os.makedirs(f"{D}/texts", exist_ok=True); os.makedirs(f"{D}/packets", exist_ok=True)
import sys; sys.path.insert(0, ROOT + "/isda2"); from i2_common import Q
P = json.load(open("edgar_pool.json")); F = {f"I{i}": dict(text=p["text"]) for i, p in enumerate(P)}
FIELDS = {n: (q, None) for n, q, _ in Q}
R = [json.loads(l) for l in open("i3_test_raw.jsonl")]; cmp_ = [r for r in R if r["event"] == "compare"]
...
rng = random.Random(20260934); sealed = {}; n = 0
for c in cmp_:
