"""A-LONG (plan step 1, Astra-signed 2026-09-24): 31 gated long contracts (along_set.json), top-2 per asked value type, NO NULL gate,
duration years->months x12 in both arms, top-5 scores logged. Derived from a3_final.py; gates as A3.
Original A3 docstring: Program A / A3 final (Astra-signed). TEST (96 contracts, frozen; one run, no re-tuning). Per contract:
...
TO = S["type_order"]; SEL = json.load(open("a1_selection.json")); tau = {t: -1.0 for t in TO}   # NO NULL gate: every asked type gets top-2
SET = json.load(open("along_set.json"))["gated"]
def mo(v): return None if v is None else ((v[0] * 12, "month") if v[1] == "year" else v)
def correct_l(t, g, p): return correct(t, mo(g), mo(p)) if t in ("Renewal Term", "Notice Period To Terminate Renewal") else correct(t, g, p)
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("along_red_sentinel.log")])
...
for ci_, cid in enumerate(SET):
    ...
    for arm in order:
        ...
        oks = [correct_l(t, g, PRED_PARSE[t](p)) for (t, g), p in zip(its, preds)]
        ...
        emit(event="req", cid=cid, arm=arm, types=qs, gold=[c["gold"][t] for t in qs], response=out, preds=preds, ok=oks, prompt_tokens=ptok,
             llm_seconds=dt, ane_seconds=ane_s if arm == "PIPE" else None, n_windows=len(ws), selected=per_type if arm == "PIPE" else None, top5={t: [[int(j), float(sc[j, TO.index(t)])] for j in np.argsort(-sc[:, TO.index(t)])[:5]] for t in qs} if arm == "PIPE" else None,
...
cids = [c for c in SET if items(C[c])]; rng = np.random.default_rng(0)
