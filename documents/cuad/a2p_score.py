...
D = "adjudication"; items = sorted(os.path.basename(p)[:-5] for p in glob.glob(f"{D}/packets/*.json"))
...
print(json.dumps(dict(event="A2P_RESULT", n_questions=N, agreement=float(np.mean([x for c in cmp_ for x in c["agree"]])), disagreements=len(items),
