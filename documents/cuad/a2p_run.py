...
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("a2p_red_sentinel.log")])
time.sleep(3); assert sent.poll() is None
assert hashlib.sha256(open("fresh/fresh60.json", "rb").read()).hexdigest().startswith("30113583867e")
POL = json.load(open("a2p_policy.json")); assert POL["pick"] == "P2"; K = 3
S = json.load(open("splits.json")); TO = S["type_order"]
F = json.load(open("fresh/fresh60.json"))["contracts"]
...
for n, f in enumerate(F):
    ...
    if n == 19 and np.mean(agree_all) < 0.80: emit(event="A2P_KILL", agreement=float(np.mean(agree_all))); break
ane.close()
