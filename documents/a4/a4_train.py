"""A4 train (Astra-signed v2): adapter AX, ONE head RESULT = window overlaps the anchor unit U of any of the paper's anchored PwC results
(a4_common, frozen). 320 TRAIN papers, DEV 40. Adapted from i2_train.py (CUA architecture; LoRA r16 q,k,v,o; host head on
[last ; mean]; positive-weighted BCE; 50% positive batches). DEV every 500 steps: hit@6 = fraction of DEV anchored results whose U overlaps
one of the top-6 windows, paper-clustered bootstrap. KILL (diagnostic) at 60 min if the DEV upper 95% < 0.70. Hard stop 3,000 steps / 90 min."""
...
assert not sh("pgrep -f 'i3_train.py|i3_test.py|a4_test.py'"), "another GPU job is running"
...
sent = subprocess.Popen([SENTINEL, str(os.getpid()), os.path.abspath("a4_red_sentinel.log")])
time.sleep(3); assert sent.poll() is None
U = json.load(open("a4_corpus.json")); SPL = json.load(open("a4_split.json"))
P = {i: U[i] for i in range(len(U))}
TCH = [dict(idx=i, split=sp, spans=[r["span"] for r in U[i]["results"]]) for sp in ("train", "dev") for i in SPL[sp]]
...
def windows(t):
    ...
    for s0 in range(0, max(1, len(ids) - (T_ - STRIDE_)), STRIDE_):
        ...
        y = np.array([1.0 if any(a_ < e[1] and b_ > e[0] for e in t["spans"]) else 0.0], np.float32)
...
class Head(nn.Module):
    def __init__(s): super().__init__(); s.l1 = nn.Linear(2 * D, 256); s.l2 = nn.Linear(256, 1)
    def __call__(s, hs, n):
...
def dev_eval():
    per = {}
    for t in DEVT:
        sc = score(t["idx"]); top = np.argsort(-sc[:, 0])[:6]
        hits = [any(W[t["idx"]][j]["a"] < e[1] and W[t["idx"]][j]["b"] > e[0] for j in top) for e in t["spans"]]
        per[t["idx"]] = hits
...
for step in range(1, STEPS + 1):
    ...
    if step % 500 == 0 or step == STEPS or due_kill or el >= 5400:
        h, ci, _ = dev_eval(); emit(event="dev", step=step, elapsed_s=time.time() - t0, hit6=h, ci=ci)
        if h > best[0]:
            best = (h, step); mx.savez("a4_best.npz", **dict(tree_flatten(net.trainable_parameters())))
            pass
        if due_kill or el >= 5400 or step == STEPS:
            if not kill_checked:
                kill_checked = True
                if ci[1] < 0.70: killed = True; emit(event="A4_KILL", step=step, hit6=h, ci=ci, note="diagnostic, not a capability verdict"); break
        if el >= 5400: emit(event="hard_stop_90min", step=step); break

emit(event="A4_TRAIN_DONE", best_step=best[1], dev_hit6=best[0], elapsed_s=time.time() - t0)
sent.terminate()
