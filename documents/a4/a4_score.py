"""A4 scoring (Astra-signed v2 + rulings): run only after a4_test.py finishes. Graded against PwC gold (numeric, rel 1e-3), no adjudication.
...
L = [json.loads(l) for l in open("a4_test_raw.jsonl")]; P = [x for x in L if x["event"] == "paper"]; done = [x for x in L if x["event"] == "A4_RUN_DONE"]
...
print(json.dumps(dict(event="A4_RESULT", papers=len(P), questions=sum(len(p["gold"]) for p in P), feasible_papers=len(F), feasible_questions=sum(len(p["gold"]) for p in F),
    acc_all=dict((a, round(acc(P, a), 4)) for a in ("PIPE", "BM25", "NONE")), acc_feasible=dict((a, round(acc(F, a), 4)) for a in ("PIPE", "FULL", "BM25", "NONE")),
    ...
    pipe_minus_bm25=round(bm, 4), pipe_minus_bm25_ci=bmci, pipe_cover=cov("pipe_cover"), bm25_cover=cov("bm25_cover"),
